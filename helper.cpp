...
void initShaders(GLuint &programID, std::string &vertFile,
                 std::string &fragFile)
{
    ...
    glDeleteShader(idHeightTexture);
}
...
void initTexture(char *height_map_file, char *texture_map_file, int *w, int *h)
{
    ...
    h_raw_image = (unsigned char *)malloc(
        h_cinfo.output_width * h_cinfo.output_height * h_cinfo.num_components);
    t_raw_image = (unsigned char *)malloc(
        t_cinfo.output_width * t_cinfo.output_height * t_cinfo.num_components);
    /* now actually read the jpeg into the raw buffer */
    h_row_pointer[0] =
        (unsigned char *)malloc(h_cinfo.output_width * h_cinfo.num_components);
    t_row_pointer[0] =
        (unsigned char *)malloc(t_cinfo.output_width * t_cinfo.num_components);
    ...
    *h = h_height;
    ...
    /* wrap up decompression, destroy objects, free pointers and close open
     * files */
